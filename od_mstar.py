...
#from config_values import sim_time
...
def find_path(obs_map, init_pos, goals, recursive=True, inflation=1.0,
    ...
    """
    #print("Inflation ....  33333333333333333-------- " + str(inflation))
    ...
    sys.setrecursionlimit(max(sys.getrecursionlimit(), longest * 25 *
                              len(init_pos)))
...
class Od_Mstar(object):
    ...
    def __init__(self, obs_map, goals, recursive, sub_search=None,
                 ...
                 end_time=200, connect_8=False, astar=False,
        ...
        tmp_width = len(self.obs_map)
        tmp_height = len(self.obs_map[0])
        tmp_workspace_1, valid_init_positions, valid_goal_positions = getWorkspace(tmp_width, tmp_height)
        tmp_neighborDirectedGraph = getNeighborGraph(tmp_width, tmp_height, tmp_workspace_1)
        self.directedNeighbors = tmp_neighborDirectedGraph.reverse()   
        #self.directedNeighbors = tmp_neighborDirectedGraph
...
#####################################################################################################################################################################
    ...
    def check_edge_cost(self, coord1, coord2):
        tmp_height = len(self.obs_map[0])
        [tmp_x, tmp_y]  = coord1
        tmp_index1 = tmp_x  + tmp_height * tmp_y + 1
        [tmp_x, tmp_y]  = coord2
        tmp_index2 = tmp_x + tmp_height * tmp_y + 1  
        tmp_value = self.directedNeighbors.get_edge_data(tmp_index2, tmp_index1)
        cost = 50000
        if tmp_value:
           cost  = 1
        if coord1 == coord2:
...
#####################################################################################################################################################################
        ...
        if self.sub_search is None:
            ...
            if self.flood_fill_policy:
                ...
                for dex, key in enumerate(self.policy_keys):
                    ...
                    #input("gen_policy_planners ... calling Astar_Graph"+str(dex) + " " + str(key))
    ...
    def get_node(self, coord, standard_node):
        ...
        #t_node.directedNeighbors = self.directedNeighbors 
        #print("Coord " +str(coord))
        #print("Col: "+str(col))
        #input("in get_node() Enter a key") 
        return t_node
    ...
    def get_step(self, init_pos, standard_node=True):
        ...
        """
        #input("In get_step...SSSSSSSSSS......."+str(init_pos))
        #input("Enter a key")  
        cur_node = self.get_node(tuple(init_pos), standard_node)
        temp = cur_node.get_step()
        if temp is not None:
            return temp
        # Use a zero time limit,  so the end time will not be modified
        path = map(list, self.find_path(tuple(init_pos), time_limit=-1))
        return list(cur_node.get_step())
    ...
    def find_path(self, init_pos, time_limit=5 * 60):
        ...
        #tmp_key = input("In OD_MSTAR_find_path... Enter a key") 
    ...
    def od_mstar_neighbors(self, node):
        ...
        #print(node.coord)
        #input("od_mstar_neighbors ... enter a key")	
        new_neighbors = []
        # visualize_holder = []
        for i in neighbors:
        ...
        #    print(str(i.coord))
        input("In od_mstar_neighbors... Enter a key") 
    ...
    def od_mstar_transition_cost(self, start_coord, prev_cost, neighbor,
        ...
        """
        prev_cost += self.sub_search[self.policy_keys[rob_dex]].get_edge_cost(start_coord[rob_dex],neighbor)
        input("...od_mstar_transition_cost.."+ str(prev_cost))  
        return prev_cost
    ...
    def get_subplanner_keys(self, col_set):
        ...
        global_col = map(lambda y: (map(lambda x: self.rob_id[x], y)),
                         col_set)
        tmp_global_col = map(list,global_col)
        global_col = list(col_set)
        
        tmp_value = []
        for i in list(col_set):
            tmp_value.append(tuple(map(lambda x: self.rob_id[x], i)))
        global_col = tuple(tmp_value)
        # generate the sub planners,  if necessary
        for dex, gc in enumerate(global_col):
            #print("gc..."+ str(gc) + " " +str(type(gc)))
            gc = tuple(gc)
            if gc not in self.sub_search:
                t_goals = tuple([self.goals[k] for k in col_set[dex]])
                self.sub_search[gc] = (self.create_sub_search(tuple(t_goals), gc))
                #input("self.sub_search[gc]: " + str(self.sub_search[gc]))

        
        return tuple(global_col)
    ...
    def get_neighbors_recursive(self, node):
        ...
        if self._col_set_memory:
            ...
            #col_set = node.col_set
            
        # Use standard operator decomposition,  if appropriate
        
        
        tmp_col_set = map(list, col_set)
        tmp_col_set = (sorted(tmp_col_set))
        if len((tmp_col_set)) == 1 and len(tmp_col_set[0]) == self.num_bots:
            # At base of recursion case
            return tuple(self.get_neighbors_nonrecursive(node))
        start_coord = node.coord
        ...
        coupled_keys = self.get_subplanner_keys((tmp_col_set))
        ...
        tmp_coupled_keys = map(list, coupled_keys)
        tmp_coupled_keys = sorted(tmp_coupled_keys)
        
        
        for i in range(len(list(tmp_coupled_keys))):
            ...
            #input("get_neighbors_recursive ........    11111111111111111") 
            try:
                  
                  new_step = tuple(self.sub_search[tuple((tmp_coupled_keys[i]))].get_step(tuple([start_coord[j] for j in list(tmp_col_set[i])])))
            ...
            for j in range(len(list(tmp_col_set[i]))):
                new_coord[tmp_col_set[i][j]] = new_step[j]
            #col_set.clear()
            #input("Strat coord:" +str(start_coord) + " New coord: "+ str(new_coord)+  "New step: "+ str(new_step))    
        del(tmp_col_set)

        new_coord = tuple(new_coord)
        #for i in range(self.num_bots):
        #    print("Coord: " +str(start_coord[i]) + " Neighbor: " + str(new_coord[i]) + "cost: " + str(self.check_edge_cost(start_coord[i],new_coord[i])))
        
        
        #input("in get_neighbors_recursive()......11111111111111111111 .........")
        # process the neighbor
        pass_col = self.pass_through(start_coord, new_coord)
        if len(pass_col) > 0:
            # Have collisions before reaching node
            return [], tuple(pass_col)
        new_node = self.get_node(new_coord, True)
        if node not in new_node.back_prop_set:
            new_node.back_prop_set.append(node)
        if not new_node.free:
            return [],  tuple(new_node.col_set)
        # Skip if closed
        if new_node.closed:
            return [],  tuple(new_node.col_set)
        ...
        #input("in get_neighbors_recursive()......22222222222222222222 .........")
        if t_cost < new_node.cost:
            new_node.cost = t_cost
            if self._col_set_memory:
                new_node.prev_col_set = effective_set
            #input("in get_neighbors_recursive()......333333333333333333333 .........")
    ...
    def od_rmstar_transition_cost(self, start_coord, prev_cost, new_coord):
        ...
        for dex, (source, target) in enumerate((zip(start_coord, new_coord))):
             ...
             prev_cost += self.sub_search[self.policy_keys[dex]].get_edge_cost(source, target)
    ...
    def get_astar_neighbors(self, node):
        ...
        #input("in get_astar_neighbors......  111111111111111")
        start_coord = node.coord
        # Generate the individually optimal setp
        base_coord = [self.sub_search[self.policy_keys[i]].get_step(
            start_coord[i]) for i in range(self.num_bots)]
        ...
        for i in to_explore:
            for bot in i:
                ...
                neighbors = self.sub_search[self.policy_keys[bot]]\
                                .get_neighbors(start_coord[bot])
                #input("Node Coord:" +str(node.coord)+ " neighbors" + str(neighbors))                
                for neigh in neighbors:
                    for k in old_coords:
                        temp = k[:]
                        temp[bot] = neigh
        ...
        neighbors = []
        old_coords = map(tuple, old_coords)
        #input("in get_astar_neighbors......  1111111222222") 
        for i in old_coords:
            ...
            if node.free:
                t_cost = node.cost
                for j in range(len(start_coord)):
                    #input("in get_astar_neighbors......  111333333333333")
                    t_cost += self.sub_search[self.policy_keys[j]].get_edge_cost(start_coord[j], i[j])
                    #t_cost += 1
                if t_cost >= new_node.cost:
                    continue
            # Check if we can get there
            ...
            if t_cost < new_node.cost:
                ...
                new_node.back_ptr = node
                neighbors.append(new_node)
        #input("Node: " + str(node.coord) + " Neighbors: " + str(neighbors))
        return tuple(neighbors), tuple(col_set)
...
class mstar_node(object):
    ...
    def backtrack_path(self, path=None, prev=None):
        ...
        if prev is not None:
            ...
            #input("Current node: "+str(self.coord) + " Cost: " + str(self.cost) + " Prev cost: " +str(prev.cost) + " " + str(self.h) + " " +str(prev.coord))             
...
def find_path_limited_graph(obs_map, init_pos, goals, recursive=True,
    ...
    # Guess that the longest path will not be any longer than 5 times the
    # longest individual robot path
    sys.setrecursionlimit(max(sys.getrecursionlimit(), longest * 10 *
                              len(init_pos)))
    path = o.find_path(init_pos, time_limit=time_limit)
    num_nodes = o.get_memory_useage(False)
    corrected_mem = o.get_memory_useage(True)
