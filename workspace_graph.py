...
class Grid_Graph(interface.Graph_Interface):
    ...
    def __init__(self, world_descriptor, diagonal_cost=False):
        ...
        tmp_workspace_1, valid_init_positions, valid_goal_positions = getWorkspace(self.width, self.height)
        tmp_neighborDirectedGraph = getNeighborGraph(self.width, self.height, tmp_workspace_1)
        #self.directedNeighbors = tmp_neighborDirectedGraph.reverse()
        self.directedNeighbors = tmp_neighborDirectedGraph     
        
        #self.graph =  self.populate_neighbors()
        #input("In grid graph....")
    def get_edge_cost(self, coord1, coord2):
        ...
        Returns edge_cost of going from coord1 to coord2.
        """
        input("4446666666666 in get_edge_cost"+ " Coord2: "+ str(coord2)+ " Coord1: "+ str(coord1)+ " Cost: "+ str(cost)) 
        ...
        if coord1 == self._goal and coord2 == self._goal:
            print("Wait cost: "+str(self._wait_cost))
            input("ENter a key ..........")  
            return self._wait_cost

        [tmp_x, tmp_y]  = coord1
        tmp_index1 = tmp_x  + self.width * tmp_y + 1
        [tmp_x, tmp_y]  = coord2
        tmp_index2 = tmp_x + self.width * tmp_y + 1   
        tmp_value =self.directedCspace.get_edge_data(tmp_index2, tmp_index1)
        if tmp_value:
           cost = tmp_value['weight']
        else:
           cost = MAX_COST 
        print("4444444444444 in get_edge_cost"+ " Coord2: "+ str(coord2)+ " Coord1: "+ str(coord1)+ " Cost: "+ str(cost))
        #input("Enter a key.......")
        return int(cost)
        #return 1
    ...
    def get_neighbors(self, coord):
        ...
        #print(neighbors)
        """
        tmp_index = coord[0] + self.width * coord[1] + 1
        tmp_neighbor_list = self.directedNeighbors.neighbors(tmp_index)
        tmp_neighbors = [] 
        for i in range(len(tmp_neighbor_list)):
            tmp_neighbor_index =  tmp_neighbor_list[i] - 1
            tmp_x =  int(tmp_neighbor_index % self.width)
            tmp_y =  int(tmp_neighbor_index / self.width)
            tmp_new_coord = (tmp_x,tmp_y)
            tmp_neighbors.append(tmp_new_coord) 
        #print(str(coord)+ " "+str(tmp_index)+" "+ str(tmp_neighbor_list)+ " " + str(tmp_neighbors))
       
        tmp_neighbors.append(coord)   
        input("in Grid_graph... get_neighbors xxxxxxxxxxxxxxxx  enter a key")
        #return neighbors
        return sorted(tmp_neighbors)
    ...
    def get_in_neighbors(self, coord):
        ...
        input("in get_in_neighbors ..AAAAAAA.. enter a key")
...
def Astar_Graph(world_descriptor, goal=None, connect_8=False,
                diagonal_cost=False, makespan=False, wait_cost=-1.0):
...
class Astar_Policy(interface.Policy_Interface):
    ...
    def __init__(self, world_descriptor, config_graph, goal=None,
        ...
        tmp_workspace_1, valid_init_positions, valid_goal_positions = getWorkspace(self.width, self.height)
        tmp_neighborDirectedGraph = getNeighborGraph(self.width, self.height, tmp_workspace_1)
        #self.directedCspace = tmp_neighborDirectedGraph.reverse()
        self.directedCspace = tmp_neighborDirectedGraph
        
        
        #print(self.directedCspace.order())
        #input("Enter a key")
...
#############################################
    ...
    def _get_node(self, coord):
        ...
        #print("In get node: "+str(node.iteration))
        if self.iteration > node.iteration:
            node.iteration = self.iteration
            node.h = self.compute_heuristic(self.init_pos, coord)
        #print("In _get_node()"+"Node coord: " + str(node.coord)+" Node policy: "+str(node.policy)+ " Heuristic: "+ str(node.h))
        #input("ENter a key")
        return node
    ...
    def _compute_path(self, coord):
        ...
        while len(self.open_list) > 0:
            ...
            #input(" Comput path Coord:" + str(node.coord) + " Neighbors: " + str(neighbors))
            for i in neighbors:
                tnode = self._get_node(i)
                #print("Coord: "+str(coord)+ "Node: "+ str(node.coord)+ "TNode: "+ str(tnode.coord)+ "Edge Cost: "+str(self.get_edge_cost(i, node.coord)))
                if (tnode.closed or tnode.cost <= node.cost + self.get_edge_cost(i, node.coord)):
                ...
                #print( " Tnode: "+ str(tnode.coord)+  "Node: " + str(node.coord) + " Policy(Next node:): "+  str(node.policy) + " Node cost: " + str(self.get_cost(node.coord)))

            #input("In compute path ... enter a key ..")
            if node.coord == coord:
    ...
    def get_in_neighbors(self, coord): 
        ...
        tmp_neighbors = [] 
        #tmp_neighbors.append(coord) 
        for i in self.directedCspace.predecessors(tmp_index):
             tmp_x =  int((i - 1) % self.width)
             tmp_y =  int((i -1) / self.width)
             tmp_new_coord = (tmp_x,tmp_y)
             #if(self.world_descriptor[tmp_x][tmp_y] != 1):
             tmp_neighbors.append(tmp_new_coord)
        #tmp_neighbors.append(coord) 
        neighbors = (tmp_neighbors)
        #input("Get in neighbors Node: "+str(coord)+ "Predecessor: " +str(neighbors)) 
        return (neighbors)  
    ...
    def get_step(self, coord):
        ...
        node = self._get_node(tuple(coord))
        
        #print("Node Coord: "+ str(node.coord))
        if node.closed:
            # Have already computed the optimal policy here
            #input("get_step .... Node Coord: "+ str(node.coord) + " Node Policy: " + str(node.policy))               
            return node.policy 
    ...
    def get_cost(self, coord):
        ...
        #input("get_cost .... Node Coord: "+ str(node.coord) + " Node Policy: " + str(node.policy)) 
        if node.closed:
            return node.cost
        self.iteration += 1
    ...
    def get_edge_cost(self, coord1, coord2):
        ...
        [tmp_x, tmp_y]  = coord2
        tmp_index2 = tmp_x + self.width * tmp_y + 1
        tmp_value =self.directedCspace.get_edge_data(tmp_index1, tmp_index2)
        ...
        if(coord1 == coord2):
            cost = 1
            #input("4444 in get_edge_cost"+ " Coord2: "+ str(coord2)+ " Coord1: "+ str(coord1)+ " Cost: "+ str(cost) + " " +str(tmp_value))
        #print("333333## in get_edge_cost"+ " Coord1: "+ str(coord1)+ " Coord2: "+ str(coord2)+ " Cost: "+ str(cost) + " " +str(tmp_value))
        return int(cost)
    ...
    def _gen_limited_offset_neighbors(self, coord):
        ...
        #print("_gen_limited_offset_neighbors...............")
        #input("Enter a key ....")
    ...
    def get_neighbors(self, coord, opt=False):
        ...
        tmp_neighbors = [] 
        #tmp_neighbors.append(coord) 
        for i in self.directedCspace.neighbors(tmp_index):
             tmp_x =  int((i - 1) % self.width)
             tmp_y =  int((i -1) / self.width)
             tmp_new_coord = (tmp_x,tmp_y)
             tmp_neighbors.append(tmp_new_coord)
        #tmp_neighbors.append(coord) 
        neighbors = (tmp_neighbors)
        #input("Coord11: "+str(coord) + "Neighbors: "+ str(neighbors))
        #print("Coord: "+str(coord))
        #print("aaaaaaaa22222  Coord: "+str(coord) + "Neighbors: "+str(neighbors))
        #input("Astar_Policy ..get_neighbors...2222222222......")
        #if not neighbors:
        #    for i in neighbors:
        #        print("robot: " + str(i) + "cost:"  + str(self.get_cost(i)))
        #input("Astar_Policy ...33333333......")
        if not opt:
            return neighbors
        for i in neighbors:
            if opt:
        ...
        #print("Opt neighbors: "+str(opt_neighbors))
        #input("Astar_Policy ...33333333......")
    ...
    def _compute_path(self, coord):
        ...
        while len(self.open_list) > 0:
            ...
            for i in neighbors:
                ...
                tmp_cost = node.cost + self.get_edge_cost(i, node.coord) 
                tnode.cost =  tmp_cost 
                tnode.policy = node.coord
                tnode.open = True
                # Can add tnode directly, and will just skip any
...
class Limited_Astar_Policy(Astar_Policy):
    ...
    def get_neighbors(self, coord):
        ...
        """
        #input("In Limited _ Astar Policy")
